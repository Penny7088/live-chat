...
class DeviceUtils {
  ...
  static Future<String?> getToken() async {
    var token = await storageKV.getString(key: StorageKey.token);
    return token;
  }
  ...
  static Future<String> getDeviceId() async {
    // 如果已经获取过设备ID，则直接返回
    if (_deviceId?.isNotEmpty == true) {
      return _deviceId!;
    }
    // 从本地获取设备ID
    var deviceId = await storageKV.getString(key: StorageKey.deviceId);
    ...
    // 保存到本地
    storageKV.setString(StorageKey.deviceId, _deviceId!);
    return _deviceId!;
  }
